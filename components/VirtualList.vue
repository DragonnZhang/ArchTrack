...
const props = defineProps<VirtualListProps<T & { node_id: string }>>()
...
const maxCount = computed(
  () => Math.ceil(props.viewHeight / props.itemHeight) + 1
)
const endIndex = computed(() =>
  Math.min(props.dataSource.length, state.startIndex + maxCount.value)
)
...
const handleScroll = () => {
  if (topDistance) {
    state.startIndex = Math.max(
      Math.floor((window.scrollY - topDistance) / props.itemHeight),
      0
    )
  }
}
